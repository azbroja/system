...
function addInvoiceProduct() {
    for (var item of invoiceProducts) {
        ...
        var addProductName = item.pivot.product_name == null ? item.name : item.pivot.product_name;
        var addProductSymbol = item.pivot.product_name == null ? '- ' + item.symbol : ' ';
        var addProductPurchasePrice = item.pivot.purchase_price == null ? '0' : item.pivot.purchase_price;
...
if (updateInvoice) {
    ...
    document.querySelector('#sendProductsForm').addEventListener('submit', function (event) {
        ...
        let customer_prices = [...document.querySelectorAll('#products-list tr')].reduce((previousValue, currentValue) => {
            ...
            if (previousValue.hasOwnProperty(productId)) {
                ...
                previousValue[productId] = {
                    ...
                    purchase_price: currentValue.querySelector('.purchasePrice').value,
